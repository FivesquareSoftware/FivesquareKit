...
#ifdef DEBUG
    NSLog(@"FSQLogger.Logging at: %d",LOG_LEVEL);
#endif
...
+ (void) trace:(NSString *)aFormat,... {
    if(LOG_TRACE >= LOG_LEVEL) {
        ...
        NSString *msg = [[[NSString alloc] initWithFormat:aFormat arguments:args] autorelease];
        va_end(args);        
...
+ (void) _logMsg:(NSString *)logMsg 
    ...
    NSData *outData = [FSQIOUtils stringData:logMsg];
    @try {
